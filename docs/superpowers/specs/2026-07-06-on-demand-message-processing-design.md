...
| **weibo** | `platforms/weibo/monitor.py` 新增 `fetch_post_by_id`(包装 `api.fetch_post_detail`) | `fetch_post_detail(cookie, post_id)` 返回 dict;判 `page_info.media_info` 有无 video → `content_type`;title 取 `clean_text[:50]`;**body 留空**(download handler `handlers.py:89-98` 已统一拉 `_fetch_long_text`,fetch 阶段重复拉会浪费配额) |
...
async def run_fetch_and_process(
    ...
    Returns:
        实际成功抓取并入库的 ID 数(供 API fetch_count 字段使用,区别于
        ``len(msg_ids)`` —— 永久失败/返回 None/未知前缀的不计入)。
...
class FetchResponse(BaseModel):
    ...
    fetch_count: int | None = None    # 恒 None(抓取异步,提交时未跑完;实际数走 SSE done 事件)
```
...
- `fetch_count` 恒为 None(抓取异步,202 提交时未跑完;实际抓取数走 SSE done 事件;引擎 `run_fetch_and_process` 返回 int 供 CLI 使用)
...
| weibo 长文 `_fetch_long_text` 失败(download 阶段) | body 为空 | fetch 阶段不调长文 API(避免与 download 阶段重复拉浪费配额,P2-1);download 阶段失败时 handler 已有降级 |
